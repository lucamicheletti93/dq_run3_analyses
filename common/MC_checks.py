...
def main():
    ...
    parser.add_argument("--run", help="Do MC checks", action="store_true")
...
def MC_checks(config):
    ...
    fInNameGen = f'{pathIn}/{prefixInGen}.root'
    fInNameRec = f'{pathIn}/{prefixInRec}.root'
    ...
    for iCut, histRec in enumerate(histRecs):
        ...
        histMassPtRecsFor1D.append(histRec.Projection(1, 0, f'{listRec}_{cuts[iCut]}_{sigRec}_MassPt_for_1D_proj'))
        histMassPtRecsFor1D[iCut].SetName(f'{listRec}_{cuts[iCut]}_{sigRec}_MassPt')
        ...
        histMassRecs.append(histMassPtRecsFor1D[iCut].ProjectionX(f'{listRec}_{cuts[iCut]}_{sigRec}_Mass'))
    ...
    # --- Integrated Axe computation with binomial uncertainties ---

    errGen = ctypes.c_double(0.0)
    errRec = ctypes.c_double(0.0)

    intGen = histPtRapGenFor1D.IntegralAndError(
        1, histPtRapGenFor1D.GetNbinsX(),
        1, histPtRapGenFor1D.GetNbinsY(),
        errGen
    )
    intRec = histPtRapRecsFor1D[0].IntegralAndError(
        1, histPtRapRecsFor1D[0].GetNbinsX(),
        1, histPtRapRecsFor1D[0].GetNbinsY(),
        errRec
    )

    axeIntegrated = (intRec / intGen) * corrFactor if intGen > 0 else 0.0
    errAxe = ( (axeIntegrated * (1.0 - axeIntegrated)) / intGen )**0.5 if intGen > 0 else 0.0

    print(f"[RESULT] Integrated Axe for {sigGen}: {axeIntegrated:.5f} ± {errAxe:.5f} (binomial)")
    ...
    canvasMass = ROOT.TCanvas("canvasMass", "", 800, 600)
    histMassRecs[0].Draw("EP")
    canvasMass.Update()
    ...
    fOutNameTxt = f'{pathOut}/{prefixOut}_{sigGen}.txt'
    ...
    fOutNameRoot = f'{pathOut}/{prefixOut}_{sigGen}.root'
